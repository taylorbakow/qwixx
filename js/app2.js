$(function() {
	...
	var lastWrapperClasses = "wrapper";
    ...
    function supportsLocalStorage() {
        return (false && typeof(Storage)!=="undefined");
    }
        ...
        if (supportsLocalStorage()) {
            if (newHighscore > localStorage.qwixx_highscore) {
                if ($(".results").hasClass("crossed-out")) {
                    ...
                    alert("New Qwixx Highscore: " + localStorage.qwixx_highscore);  
                }
            }
        }
    }
	...
	$(".result").bind("tap", function(e) {
		...
		if ($this.hasClass("finalResult")) {
			...
			$results.toggleClass("crossed-out"); 
            if ($results.hasClass("crossed-out")) {
                storeHighscore($this.html());
            }
